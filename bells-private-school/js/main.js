// Bells Private School — interaction layer.
...
function setupForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const contact = form.querySelector('[name="email"]').value.trim();
    if (!name || !contact) {
      status.textContent = "Please add your name and a phone or email so we can reach you.";
      status.style.color = "var(--ink-soft)";
      return;
    }
    status.style.color = "var(--gold-lite)";
    status.textContent = `Thank you, ${name.split(" ")[0]} — our team will be in touch shortly. For anything urgent, call +264 81 204 6275.`;
    form.reset();
  });
}
